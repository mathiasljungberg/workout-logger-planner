...
def _serialize_template(template: WorkoutTemplate) -> WorkoutTemplateRead:
...
def list_templates(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> list[WorkoutTemplateRead]:
...
@router.post("", status_code=status.HTTP_201_CREATED, response_model=WorkoutTemplateRead)
def create_template(
