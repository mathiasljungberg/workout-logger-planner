...
export function CalendarPage({ onStartSession }: CalendarPageProps) {
  ...
  useEffect(() => {
    api.get<PlannedWorkout[]>("/planned-workouts").then(setItems).catch(console.error);
  }, []);
  ...
  return (
    ...
    >
      <div className="grid">
        ...
        {items.length === 0 ? <Card>No planned workouts yet. Create a block or add one via the API.</Card> : null}
      </div>
    </Page>
  );
}
